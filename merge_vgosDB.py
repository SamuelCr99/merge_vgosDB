...
def find_history_file_name(old_file_name, inst_code):
    ...
    if "_i" in old_file_name:
        warnings.warn("File {old_file_name} already has an institution, merge this file manually")
        return old_file_name

    return f"{old_file_name.strip('.hist')}_i{inst_code}.hist"
...
def find_data_file_name(old_file_name, inst_code):
    ...
    return f"{old_file_name.strip('.nc')}_i{inst_code}.nc"
...
def handle_history_file(line, merge_directory, secondary_directory, inst_code):
    """
    ...
    if history_file_name not in os.listdir(merge_directory+'/History'):
        ...
        if not new_history_file_name:
            ...
            new_file_name = find_history_file_name(
                history_file_name, inst_code)

            if new_file_name in os.listdir(merge_directory): 
                warnings.warn("File with name {new_file_name} already exists but does not contain the same information, not merging file")
                return "", ""
...
def handle_data_file(line, merge_directory, secondary_directory, current_dir, inst_code):
    """
    ...
    if line[0] == "!" and line[1:] in os.listdir(secondary_directory):
        line = line[1:]
    ...
    if file_name in os.listdir(merge_directory):
        ...
        file_name = find_data_file_name(old_file_name,inst_code)

        # Check to make sure there no file with the institution index already exists. 
        if file_name in os.listdir(merge_directory): 
            warnings.warn("File with name {file_name} already exists but does not contain the same information, not merging file")
            return "", ""
...
def create_new_wrapper(wrapper_file, merge_directory, secondary_directory, who):
    ...
    inst_code = wrapper_file.split("_")
    for suffix in inst_code: 
        if suffix[0] == "i":
            inst_code = suffix[1:]
            break
    ...
    for line in wrapper_lines:
        ...
        if is_beginning(line):
            ...
            return_values = handle_history_file(
                line, merge_directory, secondary_directory, inst_code)
            ...
            return_values = handle_data_file(
                ...
                current_dir.get_path_with_slash(), inst_code)
